...
const Product = ({ post }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between gap-12 items-center w-full m-5">
        <div>
          <p className="text-green-600 font-semibold ">${post.price}</p>
        </div>
        {cart.some((p) => p.id === post.id) ? (
          <button className="text-gray-700 border-2 border-grey-700 rounded-full font-bold text-[12px] p-1 px-3 uppercase hover:bg-gray-700
